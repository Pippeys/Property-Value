...
library(neuralnet)
library(readxl)
Compp07_17 <- read_csv("C:/Users/SStandring/Dropbox/twerk werk/Property Value/07-17.csv", 
...
---We see there is hopeful correlation between building square footage/Floor Area Ratio and price (but they may need to be looked at in the transformed form)
```{r}
...
In the transformed graphs there is definitely some better correlation between the independent variables and transformed price. There will definitely need to be included in our model
```{r}
...
This is just some more info on the spread on the data, obviously multi-family is the large portion of our sample. And the uneven spread of the other categorical variables to property type may mean Building Class, Material, and Condition cannot be used in our analysis model.
...
#Officemod <- step(lm(Trans_Price ~ Trans_BldgSF * Cap_Rate * Trans_FAR * `Property Zip Code`, filter(Compp07_17, PropertyType == "Office")), direction = "both")
#summary(Officemod)
#plot(Officemod)
```
